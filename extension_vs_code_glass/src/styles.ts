...
// ─── Section ↔ CSS Selector Map ──────────────────────────────
//
// These target the actual DOM elements in VS Code's Electron window.
// They may change between VS Code versions — keep them updated.
...
function readSectionConfig(sectionKey: string): SectionConfig {
    const cfg = vscode.workspace.getConfiguration('glassCode');
    return {
        enabled: cfg.get<boolean>(`${sectionKey}.enabled`, true),
        opacity: cfg.get<number>(`${sectionKey}.opacity`, 0.85),
        blur:    cfg.get<number>(`${sectionKey}.blur`, 10),
    };
}
...
export function generateGlassCSS(): string {
    const cfg = vscode.workspace.getConfiguration('glassCode');
    const glassmorphism  = cfg.get<boolean>('glassmorphism', true);
    const bgImage        = cfg.get<string>('backgroundImage', '');
    const bgOpacity      = cfg.get<number>('backgroundOpacity', 0.15);
