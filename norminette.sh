...
def run(cmd):
    # Ensure we capture everything and don't let it fail silently
    p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    # If the tool crashed or reported a system error (not a code issue), return that
...
def check_clang_tidy(files: List[str]) -> Tuple[int, int]:
    ...
    msgs = []
    # FIX: We must pass flags to avoid the 'Compilation Database' error
    flags = ['--'] + EXTRA_FLAGS if EXTRA_FLAGS else ['--', '-std=c++98']

    for f in files:
        rc, out = run(['clang-tidy', f] + flags)
        # Only treat it as an issue if there's actual diagnostic output
        if "error:" in out.lower() or "warning:" in out.lower():
            msgs.append((f, out))
